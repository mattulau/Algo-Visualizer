...
- It is used to find the shortest path from the starting node to all other nodes in a graph with non-negative edge weights.
- Uses a priority queue ordered by current distance from the start.
...
6. Reset/ New graph
   ...
   - Click "back" to select a different number of nodes or a different algorithm
